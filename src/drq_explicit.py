...
def _evaluate_warmstart_warrior_multi(args_tuple):
    """
    Evaluate a single warrior against opponents in multi-warrior battle.
...
class MapElites:
    def __init__(self):
        self.archive = {}  # bc -> ExplicitWarrior
        self.history = []
...
class Main:
    """
    DRQ with explicit mutation only (no LLM).
    Fixed opponent pool, accumulating archive.
    """
    def __init__(self, args: Args):
        ...
        # Warmstart from previous run's niches (before resume check)
    ...
    def _warmstart_from_directory(self):
        ...
        Uses multiprocessing Pool to parallelize warrior evaluations.
        Each warrior is evaluated against the opponent pool in a separate process.
        """
        ...
        # Step 3: Evaluate all warriors in parallel
        print(f"Evaluating {len(eval_args)} warriors with {self.args.n_processes} processes...")
        
        with Pool(processes=self.args.n_processes) as pool:
            results = list(tqdm(
                pool.imap(_evaluate_warmstart_warrior_multi, eval_args),
                total=len(eval_args),
                desc="Warmstart evaluation"
            ))
        ...
        for result in results:
            ...
            ew = copy.deepcopy(w['ew'])
            
            if not result['success'] or result['outputs'] is None:
                failed += 1
                continue
            
            # Update warrior with outputs
            ew.outputs = result['outputs']
            ew.fitness = self.get_fitness(ew)
            ew.bc = self.get_bc_features(ew)
            
            if ew.fitness is not None and ew.fitness > -np.inf:
    ...
    def get_fitness(self, warrior: ExplicitWarrior) -> float:
        return warrior.outputs["score"].item()
    ...
    def evaluate_warrior(self, warrior: ExplicitWarrior) -> ExplicitWarrior:
        """Evaluate warrior against fixed opponent pool."""
        ...
        # Fixed opponents - no prev_champs
        opps = self.used_opps
        warriors = [warrior.warrior] + [o.warrior for o in opps]
        
        outputs = run_multiple_rounds(
            self.args.simargs, warriors, 
        ...
        if outputs is None:
            ...
            warrior.fitness = self.get_fitness(warrior)
    ...
    def carry_forward_archive(self, i_round: int):
        ...
        for warrior in carried:
            if self.args.reevaluate_carried:
                # Re-evaluate (though opponents are fixed, this ensures consistency)
                self.process_warrior(i_round, warrior)
            else:
    ...
    def init_round(self, i_round: int):
        ...
        if n_new > 0:
            ...
            for w in new_warriors:
                # Convert GPTWarrior from mutator to ExplicitWarrior
                ew = ExplicitWarrior(warrior=w.warrior, code=w.code)
    ...
    def step(self, i_round: int):
        ...
        # Update mutator with learned constants from archive
    ...
    def run(self):
        ...
        for abs_iter in pbar:
            ...
            self.used_opps = self.init_opps
            #self.used_opps = np.random.choice(self.init_opps, 12, replace=False)
    ...
    def save_final_summary(self):
        ...
        print(f"  Best fitness: {final_me.get_best().fitness:.2f}")
