"""
Explicit Core War Mutator v2 - Warmstart Optimized

Based on explicit_mutator2.py but with warmstart optimization:
- Conservative mutations (usually 1, not 1-3)
- Fine-grained constant tuning (hill climbing)
- Archive learning (extract successful constants)
- Reduced exploratory mutations
- Preserve proven structure
...
class ExplicitCoreMutator:
    """
    Explicit rule-based mutator with warmstart optimization.
...
""",
    ...
    def __init__(self, environment=None, warmstart_mode=False):
        """
        Args:
        ...
        # Select mutation weights based on mode
        if warmstart_mode:
            # WARMSTART: Conservative, fine-tuning focused
            self.mutation_weights = {
                ...
                # Conservative constant changes
                'change_constant_good': 0.08,
                'prime_step': 0.06,
                
                # Structure-preserving mutations (MEDIUM)
                'change_modifier': 0.06,
                'change_addr_mode': 0.06,
                'swap_adjacent': 0.04,
                
                # Pattern optimization (type-aware)
                'optimize_paper': 0.04,
                'optimize_stone': 0.04,
                
                # Low-weight exploration
                'change_opcode': 0.03,
            ...
            # EXPLORATION: Original v2 weights
            self.mutation_weights = {
    ...
    def learn_from_archive(self, archive: Dict):
        ...
        Call periodically with current MapElites archive.
        """
        for bc, warrior in archive.items():
            if not hasattr(warrior, 'code') or not warrior.fitness:
                continue
            if warrior.fitness <= 0:
                continue
...
"""
    ...
    async def mutate_warrior_async(self, exp_warriors: List[ExplicitWarrior], n_responses: int = 1) -> np.ndarray:
        ...
        In warmstart mode:
        - Usually only 1 mutation (80% of the time)
        - Reduced crossover probability
        ...
        for i, parent in enumerate(exp_warriors):
            for j in range(n_responses):
                for attempt in range(self.MAX_RETRIES):
                    # Crossover (reduced in warmstart mode)
                    ...
                    if len(exp_warriors) > 1 and random.random() < crossover_prob:
                        ...
                        code = getattr(parent, 'code', '') or getattr(parent, 'llm_response', '')
                    
                    # Number of mutations (conservative in warmstart)
